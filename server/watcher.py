...
from brownie.project.Project import RainOracle

from server.fire import Fire
from server.request import Request
from server.util import getWeb3Contract

class RainOracleWatcher(object):

    def __init__(self, fire:Fire, events:Dict[int, Request]):
        self._events = events
        self.active = True

        contract = getWeb3Contract(RainOracle, fire.oracle.contract.address)
        logMoveFilter = contract.events.LogRainOracleRequest.createFilter(
            fromBlock='latest')
